...
$(".jq_AddTarget").click(function () {
		...
		if(checkLogged()){
			...
			if(mainModule){
				$.post("<%=sitePathInit%>ATarget",{action:'initAddTarget'},
						   function(data){
...
$(".jq_EditTarget").click(function () {
	...
	if(checkLogged()){
		...
		$.post("<%=sitePathInit%>ATarget",{action:'initEditTarget',target_id:$(this).attr("alt")},
				   function(data){
							$('.basic-container').html(data);
							initModal();
		});
...
$(".jq_DeparmentSelect").change(function(event) {
	...
	if(checkLogged()){
		...
		$.post("<%=sitePathInit%>ATarget",{action:'loadDeparmentUser',department_id:$(this).val()},
				   function(data){
...
$(".jq_DeleteTarget").click(function () {
	...
	if(checkLogged()){
		...
		if(mainModule){
			$.post("<%=sitePathInit%>ATarget",{action:'initDeleteTarget',target_id:$(this).attr("alt")},
					   function(data){
								$('.basic-container').html(data);
								initModal();
			});
...
function loadTarget(){
	$.post("<%=sitePathInit%>ATarget",$("#formTargetList").serialize(),
			   function(data){
